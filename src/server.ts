...
export function createTanukiServer() {
  ...
  // Check for quick startup mode (used for Smithery deployment)
  const isQuickStartup = process.env.ENABLE_QUICK_STARTUP === 'true';
  
  if (isQuickStartup) {
    console.log('⚡ Quick startup mode enabled for Smithery compatibility');
  }

  // Helper function for IDE LLM mode
  async function ensureIdeLlmMode(): Promise<boolean> {
    // If in quick startup mode, return immediately for tool scanning
    if (isQuickStartup) {
      return true;
    }
  ...
  async function generateTodoWithLLM(projectDescription: string, thoughts: string): Promise<string> {
    // Skip intensive processing during quick startup
    if (isQuickStartup) {
      return "Quick startup mode: LLM todo generation will be available when fully loaded.";
    }
  ...
  async function enhanceTodoWithLLM(existingTodolist: string): Promise<string> {
    // Skip intensive processing during quick startup
    if (isQuickStartup) {
      return "Quick startup mode: LLM todolist enhancement will be available when fully loaded.";
    }
  ...
  async function findNextTaskWithLLM(todolist: string): Promise<string> {
    // Skip intensive processing during quick startup
    if (isQuickStartup) {
      return "Quick startup mode: LLM task prioritization will be available when fully loaded.";
    }
  ...
  async function createImplementationPlanWithLLM(task: string, todolistContext: string): Promise<string> {
    // Skip intensive processing during quick startup
    if (isQuickStartup) {
      return "Quick startup mode: LLM implementation planning will be available when fully loaded.";
    }
  ...
  async function parsePlanIntoActions(plan: string, task: string): Promise<any[]> {
    ...
    try {
      ...
      // Fallback function to create a minimal action set if LLM fails
      const createMinimalActionSet = () => {
        console.warn('Warning: Using fallback minimal action set');
        return Promise.resolve([
          {
            type: 'create_file',
            path: `implementation-${Date.now()}.md`,
            description: 'Documentation for task implementation',
            content: `# Implementation for: ${task}\n\n${plan}`
          }
        ]);
      };
      
      // Use the IDE's LLM for parsing
      const stdout = await executeWithIdeLlm(
        prompt,
      ...
      try {
        ...
        if (startIdx === -1 || endIdx === -1) {
          console.warn('Warning: LLM did not return a valid JSON array, using fallback');
          return createMinimalActionSet();
        }
        ...
        return validatedActions.length > 0 ? validatedActions : await createMinimalActionSet();
      } catch (error) {
        console.warn('Warning: Failed to parse LLM output as JSON, using fallback');
        return createMinimalActionSet();
      }
    } catch (error) {
      console.warn('Warning: Action extraction failed, using fallback');
      return [
        {
          type: 'create_file',
          path: `implementation-${Date.now()}.md`,
